...
#         while True:
#             #wait for processes
#             while len(my_processes) == 0:
#                 print("No process for me")
#                 new_processes = conn.root.get_processes_for_host("otho")
#                 for process in new_processes:
#                     if not process.id in id_to_process:
#                         conn.root.claim_process(process)
#                         id_to_process[process.id] = process
#                 my_processes = new_processes
#                 print("Now I have ", len(my_processes), " processes")
#                 time.sleep(1)
#             print("Found processes for me")
#             #wait for commands
#             while True:
#                 local_processes = {}
#                 for process in my_processes:
#                     print(process.id, process.manager_commands)
#                     if process.manager_commands == ManagerCommands.START:
#                         print(process.host)
...
#                             "HOME": "/homes/hriechma"}
#                         local_processes[process.id] = subprocess.Popen(process.command, cwd=process.working_directory, env=env,
#                                              stdout=std_out_file, stderr=std_err_file)
#
#                 time.sleep(1)
#                 for process_id in local_processes:
#                     if local_processes[process_id].poll() is None:
#                         print("Process ", process, "is still running")
#                     else:
#                         conn.root.process_died(id_to_process[process_id])
...
# if __name__ == "__main__":
#     import rpyc
#     conn1 = rpyc.classic.connect("otho",18812)
#     conn2 = conn1.modules["rpyc"].classic.connect("otho",44445)
#     print(dir(conn2))
#     print(dir(conn2.modules))
#     print(conn2.eval("""get_my_jobs("Test")"""))
